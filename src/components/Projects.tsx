  ...
  return (
    ...
    >
      <div className="relative h-56 overflow-hidden">
        <div className="absolute inset-0 bg-gray-900/10 group-hover:bg-gray-900/0 transition-colors z-10 pointer-events-none"></div>
          ...
          <div className="relative h-full w-full">
            ...
            {images.length > 1 && (
              <>
                <button
                  onClick={prevImage}
                  className="absolute left-2 top-1/2 -translate-y-1/2 z-20 p-1.5 rounded-full bg-black/30 hover:bg-black/50 text-white backdrop-blur-sm transition-all opacity-0 group-hover:opacity-100"
                >
                  <ArrowUpRight size={16} className="rotate-[225deg]" />
                </button>
                <button
                  onClick={nextImage}
                  className="absolute right-2 top-1/2 -translate-y-1/2 z-20 p-1.5 rounded-full bg-black/30 hover:bg-black/50 text-white backdrop-blur-sm transition-all opacity-0 group-hover:opacity-100"
                >
      ...
      <div className="p-6 flex-grow flex flex-col">
        ...
        <div className="flex items-center justify-between pt-4 border-t border-gray-100 dark:border-gray-700 mt-auto">
          ...
          <Link
            to={`/project/${project.id}`}
            className={`p-2 bg-gray-50 dark:bg-gray-700 rounded-full text-gray-500 dark:text-gray-400 transition-all duration-300 ${project.type === 'automation' ? 'group-hover:bg-emerald-600' : 'group-hover:bg-blue-600'} group-hover:text-white`}
          >
